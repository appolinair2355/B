...
async def handle_verification_code(event, client):
    ...
    try:
        ...
        # Check if message starts with 'aa' (verification code format, case-insensitive)
        if not message_text.lower().startswith('aa'):
            return False  # Not a verification code

        # Extract the actual code and strip all spaces/dashes (Telegram sometimes formats codes)
        verification_code = message_text[2:].replace(' ', '').replace('-', '').strip()

        if not verification_code.isdigit():
            await event.respond("❌ **Code invalide**\n\nLe code doit contenir uniquement des chiffres après 'aa'.\nExemple : si Telegram envoie '1 2 3 4 5', entrez **aa12345**")
            return True
...
async def store_successful_connection(user_id, phone_number):
    ...
    """
    from database import store_connection
